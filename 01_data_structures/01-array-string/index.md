...
本篇承接 [模块总览](../index.html)，先复用前置结构的接口和不变量，再学习当前结构的新增能力。读完后继续到 [链表](../02-linked-list/index.html)，把当前结构放进更大的关系或构造问题中。

## 本篇主线

数组与字符串的核心是“按位置访问一段有序数据”。连续内存让下标计算很快，但中间插入删除需要搬移元素；字符串通常不可变，修改会产生新对象。因此题目出现连续区间、双指针、前缀和或滑动窗口时，先明确边界和是否原地修改，再选择表示。

数组维护有效元素范围和长度，动态数组还要区分 size 与 capacity。字符串则维护字符序列和编码边界。后面的实现和题型都围绕这两个不变量展开。
