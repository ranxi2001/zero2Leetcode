...
本篇承接 [链表](../02-linked-list/index.html)，先复用前置结构的接口和不变量，再学习当前结构的新增能力。读完后继续到 [哈希表](../04-hash-table/index.html)，把当前结构放进更大的关系或构造问题中。

## 本篇主线

栈和队列都维护线性顺序，但取出规则不同：栈取最近加入的元素，队列取最早加入的元素。把题目中的“撤销、嵌套、等待、层序或滑动窗口”翻译成进出顺序，通常就能判断该用哪种接口。

栈的不变量是栈顶对应最后一个未取元素；队列的不变量是队首对应最早一个未取元素。双端队列允许两端操作，单调结构则在此基础上丢弃永远不可能成为答案的元素。
