...
layout: default
title: 堆
description: 从完全二叉树和数组下标出发，理解优先队列、建堆、Top K 与多路归并
eyebrow: 数据结构 / 06
permalink: /01_data_structures/06-heap/
---
...
本篇承接 [栈与队列](../03-stack-queue/index.html)，先复用前置结构的接口和不变量，再学习当前结构的新增能力。读完后继续到 [图](../07-graph/index.html)，把当前结构放进更大的关系或构造问题中。
...
最小堆要求每个父节点不大于子节点，所以根是全局最小值；最大堆反之。堆**不是有序数组**：除根和局部父子关系外，兄弟节点及不同子树之间没有排序保证。
...
堆能在 `O(log n)` 内更新**已知索引**的元素：值变小就上浮，变大就下沉。若只知道 value，先搜索它需要 `O(n)`；因此“堆支持任意删除 O(log n)”缺少已知位置这一前提。
...
1. 为什么堆可以用数组而不需要左右指针？
2. 删除堆顶时为什么要选择较小的子节点交换？
3. 自底向上建堆为什么不是 `O(n log n)`？
4. Top K 什么时候使用大小为 K 的堆？
5. 为什么优先队列需要额外 counter 才能稳定 FIFO？
