...
本篇承接 [哈希表](../04-hash-table/index.html)，先复用前置结构的接口和不变量，再学习当前结构的新增能力。读完后继续到 [并查集](../09-union-find/index.html)，把当前结构放进更大的关系或构造问题中。

## 本篇主线

Trie 把字符串的公共前缀共享成一条节点路径。它用更多节点和内存，换取按字符长度进行前缀查询、计数和补全。先区分精确匹配与前缀匹配，再决定节点是否保存子节点表、结束标志和计数。

Trie 的不变量是从根到节点的路径对应一个前缀，结束标志表示完整单词。删除不能随意断开共享前缀，只有节点不再代表任何单词且没有子节点时才可回收。
