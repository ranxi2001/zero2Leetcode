...
本篇承接 [图](../07-graph/index.html)，先复用前置结构的接口和不变量，再学习当前结构的新增能力。读完后继续到 [进阶区间结构](../10-advanced-structures/index.html)，把当前结构放进更大的关系或构造问题中。

## 本篇主线

并查集适合动态合并和查询“是否属于同一组”。它不保存完整路径，而是让每个元素沿 parent 链指向代表元；路径压缩和按大小合并共同降低长期操作成本。

并查集的核心不变量是 parent 链最终到达根，根代表一个等价类。union 连接两个根，find 在压缩路径后返回根；带权或奇偶版本则在父链上额外维护关系信息。
