...
本篇承接 [所有前置结构](../index.html)，先复用前置结构的接口和不变量，再学习当前结构的新增能力。读完后继续到 [返回总览](../模块复盘/index.html)，把当前结构放进更大的关系或构造问题中。

## 本篇主线

ACM 题目的难点常常不是算法，而是把文本输入转换成正确的数据结构。先区分 token、行和操作序列，再根据题面约定构造数组、链表、树或图；输出时还要按同一语义序列化。

构造代码的契约包括输入是否完整、编号从零还是一、空值如何表示、重复边如何处理以及异常输入如何报告。构造完成后应立即验证节点数、连接关系、根和边界值。
