...
笔试考的是限时解题能力，面试考的是**现场手撕 + 思路讲解 + 追问应对**。本模块分为四个板块：

- **计算机基础系统课**：为转码与非科班读者补齐组成原理、操作系统和计算机网络的知识主线
- **手撕代码**：面试现场手写核心代码，重点训练思路讲解、边界处理和追问应对；可额外使用 ACM 模式练习完整输入输出
- **综合测评**：性格测试、行为测评等非技术环节
- **八股题库**：学完系统课后，用通用题库、公司题库和真实面经检验表达与追问

## 计算机基础系统课 {#cs-core}

> 先理解原理，再训练 30 秒回答和连续追问。不要把题库当作第一次学习材料。

| 课程 | 学习内容 | 学完能够解释 |
|------|----------|--------------|
| [系统课总览](fundamentals/cs-core/) | 三门课依赖、统一学习方法、综合请求链路 | 怎样从定义学习到机制、实验和排障 |
| [计算机组成原理](fundamentals/cs-core/computer-organization/) | 数据与指令、CPU、流水线、Cache、TLB、中断、DMA、多核 | 程序怎样真正被硬件执行 |
| [操作系统](fundamentals/cs-core/operating-system/) | 内核、进程线程、调度同步、虚拟内存、文件与 I/O | 多个程序怎样安全共享硬件 |
| [计算机网络](fundamentals/cs-core/computer-network/) | 链路、IP 路由、TCP、DNS、HTTP、TLS 与排障 | 一次 HTTPS 请求怎样跨主机完成 |

推荐顺序：**组成原理 → 操作系统 → 计算机网络 → 通用速查题库 → 公司岗位题库**。
...
## 八股题库与岗位资料 {#fundamentals}

| 公司 | 方向 | 说明 |
|------|------|------|
| [计算机组成原理速查](fundamentals/computer-organization/) | 通用基础 | 数据表示、CPU、流水线、Cache、TLB、中断、DMA 与多核追问 |
| [操作系统速查](fundamentals/operating-system/) | 通用基础 | 进程线程、调度同步、虚拟内存、文件系统与 I/O |
| [计算机网络速查](fundamentals/computer-network/) | 通用基础 | TCP、UDP、HTTP、HTTPS、DNS 与 URL 请求链路 |
...
| [2026年2–9月后端面经八股](fundamentals/recent-2026-summer/) | 2–9月真题 | 247道题：Java、C++、Go、操作系统、网络、数据库、分布式与AI Infra |
